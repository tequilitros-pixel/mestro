...
export async function POST(request: Request) {
  try { const actor = await getCurrentCommandActor(); const terminalId = await requireTerminalRequest(request); const body = await request.json(), branchId = String(body.branchId), registerId = String(body.registerId); await requireTerminalBranch(terminalId, branchId, registerId); const outcome = await createOrder({ actor, terminalId, operationId: String(body.operationId), branchId, registerId, cashSessionId: String(body.cashSessionId), expiresAt: body.expiresAt ? new Date(body.expiresAt) : null, reference: body.reference === undefined ? null : String(body.reference) }); return NextResponse.json(outcome.result, { status: outcome.replayed ? 200 : 201 }); } catch (error) { return pos2ErrorResponse(error); }
}
