...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const outcome = await command({ operationId: body.operationId, cashSessionId: id, terminalId, actor, amount: String(body.amount), reason: String(body.reason ?? ""), categoryId: typeof body.categoryId === "string" ? body.categoryId : undefined, metadata: typeof body.receiptPhotoUrl === "string" ? { receiptPhotoUrl: body.receiptPhotoUrl } : undefined });
