...
export default async function NewLiquorProductPage() {
  async function createProduct(formData: FormData) {
    ...
    const rawPrefix = String(formData.get("prefix") ?? "").trim();
    const description =
    ...
    if (!slug) {
      throw new Error("No fue posible generar un slug válido para el producto.");
    }
    ...
    if (!prefix) {
      throw new Error(
        "El prefijo es obligatorio. Ejemplo: usa \"LZ\" para Licor de Zarzamora."
      );
    }
    ...
    if (
      ...
      throw new Error("El porcentaje de alcohol debe estar entre 0 y 100.");
    }
    ...
    if (redAlertDays <= 0) {
      throw new Error("Los días de alerta roja deben ser mayores que cero.");
    }
    ...
    try {
      const product = await prisma.liquorProduct.create({
        data: {
          name,
          slug,
          prefix,
      ...
      if (
        ...
        if (target.includes("slug")) {
          throw new Error(
            "Ya existe un producto con ese slug. Elige otro identificador de URL."
          );
        }

        if (target.includes("prefix")) {
          throw new Error(
            "Ya existe un producto con ese prefijo. Elige otro código, por ejemplo cambia \"LZ\" por \"LZ2\"."
          );
        }

        throw new Error("Ya existe un producto con esos datos.");
      }
    ...
    redirect(`/liquors/products/${createdSlug}`);
  }
      ...
      <header className="mt-6 rounded-3xl border border-outline-variant bg-surface-container p-6 sm:p-8">
        ...
        <div className="mt-4 flex items-start gap-4">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border border-outline-variant bg-surface-container-high text-4xl">
            <MartiniIcon className="h-7 w-7 text-on-surface-variant" />
          </div>
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-on-surface-variant">
              Registra un licor nuevo. Después podrás crear su receta oficial
      ...
      <form action={createProduct} className="mt-6 space-y-6">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            >
              <input
                ...
                type="text"
                required
                placeholder="Licor de Zarzamora"
            ...
            <Field
              label="Prefijo de lote"
              description={
                'Código corto usado en los folios de lote. Ejemplo: "LZ" genera lotes como LZ-21-07-2026-013.'
              }
            >
              <input
                name="prefix"
            ...
            <Field
              label="Slug (URL)"
              description="Déjalo en blanco para generarlo automáticamente a partir del nombre."
            >
              <input
                name="slug"
                type="text"
        ...
        <div className="sticky bottom-4 z-20 flex flex-col-reverse gap-3 rounded-2xl border border-outline-variant bg-background/95 p-4 shadow-2xl backdrop-blur sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-outline">
            Después de crear el producto podrás registrar su receta oficial.
          </p>
