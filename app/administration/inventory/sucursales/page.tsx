...
const formatDate = (value: Date) =>
  new Intl.DateTimeFormat("es-MX", { day: "2-digit", month: "short" }).format(value);
...
const sections: {
  ...
  {
    title: "Stock actual",
    description: "Stock estimado por sucursal y producto, con alertas de mínimos.",
    href: "/administration/inventory/sucursales/stock",
    icon: ChartBarIcon,
  },
  {
    title: "Entradas y ajustes",
    description: "Registra compras y ajustes (incluyendo mermas o pérdidas) por sucursal.",
    href: "/administration/inventory/branch-entries",
    icon: InboxIcon,
  },
  {
    title: "Traspasos entre sucursales",
    description: "Mueve stock de una sucursal a otra sin perder el rastro.",
    href: "/administration/inventory/sucursales/traspasos",
    icon: ArrowsRangeIcon,
  },
  {
    title: "Conteos semanales",
    description: "Conteo por sucursal, cada lunes, para calcular el consumo de la semana.",
...
const quickActions = [
  ...
  { label: "Nuevo traspaso", href: "/administration/inventory/sucursales/traspasos" },
  { label: "Nuevo conteo", href: "/administration/inventory/branch-counts/new" },
];
...
export default async function SucursalesInventoryPage() {
  ...
  const lowStockProducts = products
    ...
    .filter((product) => Number(product.minimumStock) > 0 && product.stock < Number(product.minimumStock))
  ...
  const trendData = Array.from(trendMap.entries()).map(([key, count]) => ({
    ...
    label: new Intl.DateTimeFormat("es-MX", { day: "2-digit" }).format(new Date(`${key}T00:00:00`)),
    movimientos: count,
  }));
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold sm:text-4xl">Inventario de sucursales</h1>
              ...
              {lowStockProducts.slice(0, 8).map((product) => (
                <div key={product.id} className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-error">
                    {product.stock.toFixed(1)} / min {Number(product.minimumStock)} {product.unit}
                  </span>
                </div>
              ))}
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {recentEntries.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-2 py-6 text-center text-on-surface-variant">
                ...
                {recentEntries.map((entry) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-2 py-3">{formatDate(new Date(entry.entryDate))}</td>
                      <td className="px-2 py-3">{entryTypeLabels[entry.type] ?? entry.type}</td>
                      <td className="px-2 py-3">{entry.product.name}</td>
                      <td className="px-2 py-3">{entry.branch.name}</td>
                      <td
                        className={`px-2 py-3 text-right ${
                          quantity < 0 ? "text-error" : "text-on-surface"
                        }`}
                      >
                        {quantity > 0 ? "+" : ""}
