  ...
  return (
    ...
    >
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <div className="space-y-2 md:col-span-2"><span className="text-sm font-semibold text-on-surface-variant">Contenido de cada presentación</span><div className="grid gap-3 sm:grid-cols-2"><input name="contentPerUnit" type="number" min="0.001" step="0.001" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Ej. 600, 1800 o 1.8" className="w-full rounded-xl border border-outline-variant bg-background px-4 py-3 text-sm" /><select name="contentUnit" value={contentUnit} onChange={(event) => setContentUnit(event.target.value)} className="w-full rounded-xl border border-outline-variant bg-background px-4 py-3 text-sm"><option value="ML">ml</option><option value="L">L</option><option value="G">g</option><option value="KG">kg</option><option value="PIEZAS">piezas</option></select></div><p className="text-xs text-on-surface-variant">{formatCommercialPresentation({ handlingUnit: unit === "Paquete" ? "PAQUETE" : unit.toUpperCase(), contentPerUnit: content ? Number(content) : null, contentUnit }) ?? "Presentación sin configurar"}</p></div>
