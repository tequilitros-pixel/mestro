...
import { captureInitialInventoryCount } from "@/lib/pos2/inventory/initialCount";
...
async function actor(){const u=await requireAdminAction();return{id:u.id,role:u.role,branchIds:null}as const;} const refresh=()=>revalidatePath("/administration/pos2/inventory"); const unit=(f:FormData):CatalogBaseUnit=>f.get("unit")==="ML"?"ML":f.get("unit")==="G"?"G":"UNIT";
...
export async function captureInitialCountAction(f:FormData){await captureInitialInventoryCount({actor:await actor(),operationId:String(f.get("operationId")),branchId:String(f.get("branchId")),inventoryProductId:String(f.get("inventoryProductId")),countedUnits:String(f.get("countedUnits")),captureUnit:String(f.get("captureUnit")) as "BASE"|"PRESENTATION",notes:String(f.get("notes")||"")});refresh();}
