...
export default async function Pos2CashAdministrationPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-8 p-6">
    ...
    <section className="grid gap-4 md:grid-cols-2">
      ...
      <TerminalEnrollmentForm branches={branches} terminals={terminals.map((terminal) => ({ id: terminal.id, branchId: terminal.branchId, branchName: terminal.branch.name, name: terminal.name, status: terminal.status, credentialIssuedAt: terminal.credentialIssuedAt?.toISOString() ?? null }))} />
    </section>
    ...
    <section><h2 className="mb-3 text-lg font-semibold">Terminales</h2><div className="space-y-2">{terminals.map((terminal) => <div key={terminal.id} className="flex items-center justify-between gap-4 rounded-lg border p-3"><span>{terminal.branch.name} · {terminal.name} · {terminal.status} · Última credencial: {terminal.credentialIssuedAt ? formatBusinessDateTime(terminal.credentialIssuedAt) : "—"}</span>{terminal.status !== "REVOKED" && <form action={revokeTerminalAction}><input type="hidden" name="terminalId" value={terminal.id}/><button className="text-sm text-error underline">Revocar</button></form>}</div>)}</div></section>
    <section><h2 className="mb-3 text-lg font-semibold">Sesiones recientes</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left"><th>Sucursal</th><th>Register</th><th>Estado</th><th>Apertura</th><th>Esperado</th><th>Diferencia</th></tr></thead><tbody>{sessions.map((session) => <tr key={session.id} className="border-t"><td>{session.branch.name}</td><td>{session.register.name}</td><td>{session.status}</td><td>{formatBusinessDateTime(session.openedAt)}</td><td>{session.expectedCash?.toString() ?? "—"}</td><td>{session.difference?.toString() ?? "—"}</td></tr>)}</tbody></table></div></section>
  </main>;
}
