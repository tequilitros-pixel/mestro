...
export function TerminalEnrollmentForm({ branches, terminals }: { branches: Array<{ id: string; name: string }>; terminals: Array<{ id: string; branchId: string; branchName: string; name: string; status: string }> }) {
  ...
  return <div className="space-y-4 rounded-xl border border-outline-variant p-4">
    ...
    <form action={action} className="space-y-3 border-t border-outline-variant pt-4">
      ...
      {state.credential && <div className="rounded-lg bg-surface-container p-3"><p className="text-sm font-medium">Credencial de un solo uso para {state.terminalName}</p><input aria-label="Credencial emitida" readOnly value={state.credential} className="w-full rounded border bg-white p-2 font-mono text-sm"/><p className="mt-1 text-xs text-on-surface-variant">Se muestra una sola vez; mantenla solo en el dispositivo autorizado.</p></div>}
