...
export async function createPriceAction(form: FormData) {
  ...
  await createPriceVersion({ ...target, ...scope, actor: await actor(), operationId: String(form.get("operationId")), amount: String(form.get("amount")), validFrom: parseBusinessDateTimeLocal(String(form.get("validFrom"))), validTo: validTo ? parseBusinessDateTimeLocal(validTo) : null });
  refresh();
}

export async function endPriceAction(form: FormData) {
  await endPriceVersion({ actor: await actor(), operationId: String(form.get("operationId")), priceVersionId: String(form.get("priceVersionId")), effectiveAt: parseBusinessDateTimeLocal(String(form.get("effectiveAt"))), reason: String(form.get("reason")) });
  refresh();
}
