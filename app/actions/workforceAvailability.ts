...
function destination(formData: FormData, attention: { required: boolean }) {
  const returnTo = String(formData.get("returnTo") ?? "/workforce/availability");
...
export async function saveWorkforceAvailabilityRuleAction(formData: FormData) {
  ...
  revalidatePath("/workforce/availability");
  revalidatePath("/administration/workforce/availability");
