...
export default function CashCutDetailPage() {
  ...
  if (isClosed) {
    return <ClosedCutSummaryRefactored cashCut={cashCut} />;
  }

  const steps: Step[] = ["ventas", "salidas", "entradas", "evidencias", "cierre"];
  const statuses = Object.fromEntries(
    steps.map((s) => [s, getStepStatus(s, cashCut)])
  ) as Record<Step, "complete" | "empty" | "optional">;
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <CompactStepper current={step} statuses={statuses} onSelect={setStep} />
      ...
      <StepFooterNav current={step} onSelect={setStep} steps={steps} />
    </div>
  );
}

// Los componentes Step siguen igual...
// VentasStep, SalidasStep, EntradasStep, EvidenciasStep, CierreStep se mantienen sin cambios
// (por brevedad, omito aquí pero se copian del original)
...
function VentasStep({ cashCutId, cashCut, onSaved, disabled }: StepProps) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-on-surface-variant">Si usaste el POS, estos montos ya están cargados — escribir aquí <span className="font-semibold">reemplaza</span> (no suma).</p>
      {METODOS.map((m) => (
        <Card key={m.key}>
      ...
      <Card highlight>
        ...
        <CardValue>{formatMoney(total)}</CardValue>
      </Card>
        ...
        {cashCut.outflows.map((o) => (
          <Card key={o.id}>
            <div className="flex justify-between">
              ...
              <p className="text-on-surface font-bold">{formatMoney(o.amount)}</p>
            </div>
          </Card>
        ))}
      </div>
        ...
        {cashCut.inflows.map((i) => (
          <Card key={i.id}>
            <div className="flex justify-between">
              ...
              <p className="text-on-surface font-bold">{formatMoney(i.amount)}</p>
            </div>
          </Card>
        ))}
      </div>
...
function CierreStep({ cashCutId, cashCut, onSaved }: StepProps) {
  ...
  const totalVentas = cashCut.salesByMethod.reduce((sum, s) => sum + s.amount, 0);
  const canClose = true;
      ...
      <Card>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2 text-sm">
            <CheckIcon className="h-4 w-4 text-tertiary-fixed-dim" />
            <span className="text-on-surface">Ventas: ${totalVentas.toFixed(2)}</span>
          </div>
        </div>
      </Card>
