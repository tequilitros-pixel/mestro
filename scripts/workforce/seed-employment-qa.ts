...
async function main() {
  ...
  try {
    ...
    const qaUser = (await client.query(`SELECT id FROM "User" WHERE active=true AND (username LIKE 'wfqa-%' OR name LIKE 'QA %') ORDER BY id LIMIT 1`)).rows[0];
    ...
    await client.query(`INSERT INTO "BranchAssignment" (id,"employmentId","branchId",type,"effectiveFrom","createdAt","updatedAt") VALUES ($1,$2,$3,'ALLOWED',$4,now(),now()),($5,$2,$6,'ALLOWED',$4,now(),now())`, [id("allowed", 1), id("employment", 2), branches[0].id, new Date("2025-01-01T00:00:00Z"), id("allowed", 2), branches[1].id]);
    ...
    console.log(JSON.stringify({ environment: "verified DEV", prefix: PREFIX, employees: 6, branches: branches.map((branch) => branch.name), userRowsTouched: 0, branchRowsTouched: 0, employmentRows: 6, homeAssignments: 6, allowedAssignments: 2, payRates: 7 }, null, 2));
