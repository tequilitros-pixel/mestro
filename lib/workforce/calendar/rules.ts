...
export function latestPublishedRevisions(rows: PublishedRevision[]) {
  ...
  for (const row of [...rows].sort(
    ...
    if (!latest.has(row.shiftId)) latest.set(row.shiftId, row);
...
export function calendarStatus(
  row: Pick<PublishedRevision, "revisionStatus" | "revisionNumber">,
) {
  return row.revisionStatus === "CANCELLED"
    ? ("CANCELLED" as const)
...
export function effectiveCalendarStatus(input: {
  status: "DRAFT" | "PUBLISHED" | "CANCELLED";
  hasPublicationLink: boolean;
  latestRevisionLinked: boolean;
}) {
  if (input.status === "CANCELLED") return "CANCELLED" as const;
  return input.hasPublicationLink && !input.latestRevisionLinked
    ? ("CHANGED" as const)
    : ("NEW" as const);
}

export function staysOnBusinessDate(
  row: Pick<PublishedRevision, "businessDate">,
) {
