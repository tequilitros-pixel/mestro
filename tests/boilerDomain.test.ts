...
test("el parche separa el cierre de consumidor del cierre global y conserva el contexto", () => {
  const service = fs.readFileSync("lib/boiler/service.ts", "utf8");
  const sync = fs.readFileSync("app/api/sync/operations/route.ts", "utf8");
  const boilerPage = fs.readFileSync("app/boiler/[id]/page.tsx", "utf8");
  const cookingPage = fs.readFileSync("app/cooking/[id]/page.tsx", "utf8");
  assert.match(service, /processId: cooking\.id, processType: "COCIMIENTO", endedAt: null/);
  assert.match(service, /export async function closeCookingSteamState/);
  assert.match(cookingPage, /closeCookingSteamState\(transaction, id, finishedAt\)/);
  assert.match(service, /EVENT_TRANSITION_NOT_ALLOWED/);
  assert.match(sync, /error instanceof BoilerDomainError/);
  assert.match(boilerPage, /name="sessionId" value=\{session\.id\}/);
});

test("las pestañas no envían accidentalmente los formularios server-action", () => {
  const tabs = fs.readFileSync("components/ui/PageTabs.tsx", "utf8");
  assert.match(tabs, /<button\s+key=\{t\.key\}\s+type="button"/);
});

test("el evento genérico solo permite observaciones", () => {
  const page = fs.readFileSync("app/boiler/[id]/page.tsx", "utf8");
  const service = fs.readFileSync("lib/boiler/service.ts", "utf8");
  assert.match(page, /<option value=\{BoilerEventType\.OBSERVACION\}>OBSERVACION<\/option>/);
  assert.match(service, /new Set<BoilerEventType>\(\[BoilerEventType\.OBSERVACION\]\)/);
  for (const type of ["MANTENIMIENTO", "INCIDENTE", "RECARGA_GAS", "CAMBIO_PRESION", "LECTURA_GAS"]) {
    assert.doesNotMatch(page, new RegExp(`BoilerEventType\\.${type}`));
  }
});

test("el horómetro de pantalla usa el agregado completo y el historial sigue limitado", () => {
  const page = fs.readFileSync("app/boiler/[id]/page.tsx", "utf8");
  assert.match(page, /COALESCE\(SUM\(EXTRACT\(EPOCH FROM/);
  assert.match(page, /boilerSession\.findMany\(\{ where: \{ equipmentId: id, endedAt: \{ not: null \} \}/);
  assert.match(page, /take: 20, include: sessionInclude/);
  assert.match(page, /horometerHours\.toFixed\(2\)/);
  assert.match(page, /sessions\.length \? sessions\.map/);
});
