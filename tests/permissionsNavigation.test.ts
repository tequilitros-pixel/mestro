...
test("Caldera aparece en Producción y conserva autorización específica", () => {
  const boiler = SUBMENUS.production.find((item) => item.href === "/boiler")!;

  assert.equal(boiler.label, "Caldera");
  assert.equal(getModuleKeyForPath("/boiler/cmtyp19cv0001eotwpab7q7w1"), "/boiler");
  assert.equal(isMainModuleVisible("ADMIN", [], MAIN_MODULES.find((m) => m.module === "production")!), true);
  assert.equal(isSubmenuItemVisible("GERENTE", ["/boiler"], boiler), true);
  assert.equal(isSubmenuItemVisible("GERENTE", [], boiler), false);
  assert.equal(isSubmenuItemVisible("GERENTE", ["/cooking"], boiler), false);
  assert.equal(canAccessModule("GERENTE", ["/boiler"], "/boiler"), true);
  assert.equal(canAccessModule("GERENTE", [], "/boiler"), false);
  assert.equal(canAccessModule("GERENTE", ["/cooking"], "/boiler"), false);
});
